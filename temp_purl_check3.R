...
## ----glycemic-by-hypertension-history, warning=FALSE, message=FALSE, echo=FALSE----
hypertension_history_plot_data <- a1c_all_a1cdemo_dedup %>%
  dplyr::filter(!is.na(age_group), !is.na(glycemic_category), glycemic_category != "hypoglycemia") %>%
  dplyr::mutate(
    hypertension_history = dplyr::case_when(
      hypertension == 1 ~ "History of hypertension",
      TRUE ~ "No history of hypertension"
    ),
    hypertension_history = factor(
      hypertension_history,
      levels = c("No history of hypertension", "History of hypertension")
    )
  ) %>%
  dplyr::group_by(age_group, hypertension_history, glycemic_category) %>%
  dplyr::summarise(group_count = dplyr::n(), .groups = "drop") %>%
  dplyr::left_join(
    a1c_all_a1cdemo_dedup %>%
      dplyr::filter(!is.na(age_group), !is.na(glycemic_category), glycemic_category != "hypoglycemia") %>%
      ...
      dplyr::group_by(age_group, hypertension_history) %>%
      dplyr::summarise(total_donors = dplyr::n(), .groups = "drop"),
    by = c("age_group", "hypertension_history")
  ) %>%
  dplyr::mutate(percentage = group_count / total_donors)

hypertension_history_glycemic_tab <- xtabs(
  ~ interaction(age_group, hypertension_history) + glycemic_category,
  data = a1c_all_a1cdemo_dedup %>%
    dplyr::filter(!is.na(age_group), !is.na(glycemic_category), glycemic_category != "hypoglycemia") %>%
    dplyr::mutate(
      hypertension_history = dplyr::case_when(
...
hypertension_history_glycemic_chisq <- chisq.test(hypertension_history_glycemic_tab)

knitr::kable(
  hypertension_history_plot_data %>%
    dplyr::mutate(
      percentage = scales::percent(percentage, accuracy = 0.1)
    ) %>%
    dplyr::select(
  ...
  caption = "Glycemic status by age group stratified by hypertension history."
)

ggplot(hypertension_history_plot_data, aes(x = age_group, y = percentage)) +
  geom_bar(
    aes(color = glycemic_category, fill = glycemic_category),
    stat = "identity",
    position = position_dodge2(0.8),
  ...
  facet_wrap(~ hypertension_history) +
  geom_text(
    aes(
      label = paste0(round(100 * percentage, 1), "%"),
      group = glycemic_category
  ...
  scale_y_continuous(labels = scales::percent, limits = c(0, 1.05)) +
  labs(
    title = "Glycemic status of donors by age group stratified by hypertension history",
    subtitle = paste(
      "among donors screened in 2025-2026;",
      "chi-square p",
      scales::pvalue(hypertension_history_glycemic_chisq$p.value, accuracy = 0.001)
    ),
    caption = "donors with A1c% below 4.0 are excluded",
    x = "donor age group",
    y = "percentage of donors"
...
## ----glycemic-by-bp-hypertension-stage, warning=FALSE, message=FALSE, echo=FALSE----
bp_stage_plot_data <- a1c_all_a1cdemo_dedup %>%
  dplyr::filter(
    !is.na(age_group),
    !is.na(glycemic_category),
    glycemic_category != "hypoglycemia",
    BP_category %in% c("Hypertension stage 1", "Hypertension stage 2")
  ) %>%
  dplyr::mutate(
    bp_hypertension_stage = factor(
      as.character(BP_category),
      levels = c("Hypertension stage 1", "Hypertension stage 2")
    )
  ) %>%
  dplyr::group_by(age_group, bp_hypertension_stage, glycemic_category) %>%
  dplyr::summarise(group_count = dplyr::n(), .groups = "drop") %>%
  dplyr::left_join(
    a1c_all_a1cdemo_dedup %>%
      dplyr::filter(
      ...
      dplyr::group_by(age_group, bp_hypertension_stage) %>%
      dplyr::summarise(total_donors = dplyr::n(), .groups = "drop"),
    by = c("age_group", "bp_hypertension_stage")
  ) %>%
  dplyr::mutate(percentage = group_count / total_donors)

bp_stage_glycemic_tab <- xtabs(
  ~ interaction(age_group, bp_hypertension_stage) + glycemic_category,
  data = a1c_all_a1cdemo_dedup %>%
    dplyr::filter(
      !is.na(age_group),
      !is.na(glycemic_category),
...
bp_stage_glycemic_chisq <- chisq.test(bp_stage_glycemic_tab)
...
knitr::kable(
  bp_stage_plot_data %>%
    ...
    dplyr::select(
      `Age group` = age_group,
      `Measured hypertension stage` = bp_hypertension_stage,
      `Glycemic category` = glycemic_category,
      `Donors in category` = group_count,
      `Total donors in stratum` = total_donors,
      Percentage = percentage
    ),
  caption = "Glycemic status by age group stratified by measured hypertension stage."
)

ggplot(bp_stage_plot_data, aes(x = age_group, y = percentage)) +
  geom_bar(
    aes(color = glycemic_category, fill = glycemic_category),
    stat = "identity",
    position = position_dodge2(0.8),
  ...
  facet_wrap(~ bp_hypertension_stage) +
  geom_text(
    aes(
      label = paste0(round(100 * percentage, 1), "%"),
      group = glycemic_category
  ...
  scale_y_continuous(labels = scales::percent, limits = c(0, 1.05)) +
  labs(
    title = "Glycemic status of donors by age group stratified by measured hypertension stage",
    subtitle = paste(
      "among donors screened in 2025-2026;",
      "chi-square p",
      scales::pvalue(bp_stage_glycemic_chisq$p.value, accuracy = 0.001)
    ),
    caption = "donors with A1c% below 4.0 are excluded; restricted to donors in BP categories Hypertension stage 1 or stage 2",
    x = "donor age group",
    y = "percentage of donors"
  ) +
  customeTheme +
