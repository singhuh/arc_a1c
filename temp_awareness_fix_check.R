...
load("survey_a1c_demo.Rdata")
if (is.environment(survey_a1c_demo) && exists("survey_a1c_demo", envir = survey_a1c_demo)) {
  survey_a1c_demo <- get("survey_a1c_demo", envir = survey_a1c_demo)
}

survey_question_vars <- c(
  "q1_a1c_knowledge",
  "q3_prior_a1c_awareness"
)

survey_question_order <- list(
  q1_a1c_knowledge = c("Yes", "No", "Unsure"),
  q3_prior_a1c_awareness = c("Yes", "No", "Unsure")
)

build_survey_hyperglycemic_distribution <- function(df, question_var) {
  response_values <- as.character(df[[question_var]])
  glycemic_values <- as.character(df$glycemic_category)

  valid_rows <- !is.na(response_values) &
    trimws(response_values) != "" &
    !is.na(glycemic_values) &
    trimws(glycemic_values) != ""

  response_values <- response_values[valid_rows]
  glycemic_values <- glycemic_values[valid_rows]

  response_levels <- survey_question_order[[question_var]]
  observed_levels <- sort(unique(response_values))
  response_levels <- unique(c(response_levels, setdiff(observed_levels, response_levels)))

  response_factor <- factor(response_values, levels = response_levels)
  hyperglycemic_values <- dplyr::case_when(
    glycemic_values == "Normoglycemia" ~ "Normoglycemia",
    glycemic_values %in% c("Prediabetes", "Diabetes") ~ "Hyperglycemia",
    TRUE ~ NA_character_
  )
  hyperglycemic_factor <- factor(
    hyperglycemic_values,
    levels = c("Normoglycemia", "Hyperglycemia")
  )

  counts_matrix <- table(response_factor, hyperglycemic_factor)
  counts_df <- as.data.frame.matrix(counts_matrix, stringsAsFactors = FALSE)
  for (column_name in c("Normoglycemia", "Hyperglycemia")) {
    if (!column_name %in% names(counts_df)) {
      counts_df[[column_name]] <- 0L
    }
  }
  counts_df$response <- rownames(counts_df)
  rownames(counts_df) <- NULL

  counts_df$response_n <- rowSums(counts_df[, c("Normoglycemia", "Hyperglycemia"), drop = FALSE])
  counts_df <- counts_df[counts_df$response_n > 0, , drop = FALSE]

  counts_df <- counts_df %>%
    dplyr::mutate(
      normoglycemia_pct = 100 * Normoglycemia / response_n,
      hyperglycemia_pct = 100 * Hyperglycemia / response_n
    )

  question_label <- attr(df[[question_var]], "label")
  if (is.null(question_label) || !nzchar(question_label)) {
    question_label <- question_var
  }

  chisq_p_value <- tryCatch(
  ...
  list(
    ...
    question_n = sum(counts_df$response_n),
    chisq_p_value = chisq_p_value,
    distribution = counts_df
  )
}

survey_hyperglycemic_results <- lapply(
  survey_question_vars,
  function(question_var) build_survey_hyperglycemic_distribution(survey_a1c_demo, question_var)
)
names(survey_hyperglycemic_results) <- survey_question_vars

survey_hyperglycemic_overview <- dplyr::bind_rows(
  lapply(survey_hyperglycemic_results, function(result) {
    result$distribution %>%
      dplyr::transmute(
        response,
        hyperglycemia_pct
      ) %>%
      dplyr::arrange(dplyr::desc(hyperglycemia_pct), response) %>%
      dplyr::slice(1) %>%
      dplyr::transmute(
        question = result$question_label,
        donors = result$question_n,
        highest_hyperglycemia_response = response,
        highest_hyperglycemia_pct = hyperglycemia_pct,
        chisq_p_value = result$chisq_p_value
      )
  })
)

survey_hyper_prior_a1c_data <- survey_a1c_demo %>%
  dplyr::filter(
    glycemic_category %in% c("Prediabetes", "Diabetes"),
    !is.na(q3_prior_a1c_awareness)
  ) %>%
  dplyr::mutate(
    q3_prior_a1c_awareness = factor(
      q3_prior_a1c_awareness,
      levels = c("No", "Unsure", "Yes")
    )
  )

survey_hyper_prior_a1c_summary <- survey_hyper_prior_a1c_data %>%
  dplyr::summarise(
    hyper_n = dplyr::n(),
    no_pct = 100 * mean(q3_prior_a1c_awareness == "No"),
    unsure_pct = 100 * mean(q3_prior_a1c_awareness == "Unsure"),
    yes_pct = 100 * mean(q3_prior_a1c_awareness == "Yes"),
    unaware_or_unsure_pct = 100 * mean(q3_prior_a1c_awareness %in% c("No", "Unsure")),
    current_a1c_mean = mean(a1c, na.rm = TRUE),
    current_a1c_median = median(a1c, na.rm = TRUE),
    current_a1c_q1 = quantile(a1c, 0.25, na.rm = TRUE),
    current_a1c_q3 = quantile(a1c, 0.75, na.rm = TRUE)
  )

survey_hyper_prior_a1c_plot_data <- survey_hyper_prior_a1c_data %>%
  dplyr::count(q3_prior_a1c_awareness, name = "donors") %>%
  dplyr::arrange(q3_prior_a1c_awareness) %>%
  dplyr::mutate(
    percentage = 100 * donors / sum(donors),
    category = as.character(q3_prior_a1c_awareness)
  ) %>%
  dplyr::select(category, donors, percentage)

survey_hyper_prior_a1c_assoc_data <- tibble::tibble()
survey_hyper_race_awareness_summary <- tibble::tibble()
survey_hyper_urban_awareness_summary <- tibble::tibble()
survey_hyper_race_urban_awareness_summary <- tibble::tibble()
survey_hyper_race_awareness_chisq <- NULL
survey_hyper_urban_awareness_chisq <- NULL
survey_hyper_race_urban_awareness_chisq <- NULL
survey_hyper_awareness_assoc_plot_data <- tibble::tibble()

render_survey_hyperglycemic_gt <- function(question_var) {
  result <- survey_hyperglycemic_results[[question_var]]
  ...
  result$distribution %>%
    ...
    gt::gt() %>%
    gt::tab_header(
      title = gt::md(result$question_label),
      subtitle = paste0(
        "Donors with nonmissing response and glycemic category (n = ",
        scales::comma(result$question_n),
        "); Pearson chi-square p ",
        scales::pvalue(result$chisq_p_value, accuracy = 0.001)
      )
    ) %>%
    gt::cols_label(
      response = "Response",
      response_n = "Donors",
      Normoglycemia = "Normoglycemia n",
      normoglycemia_pct = "Normoglycemia %",
      Hyperglycemia = "Hyperglycemia n",
      hyperglycemia_pct = "Hyperglycemia %"
    ) %>%
    gt::fmt_number(
      columns = c(Normoglycemia, Hyperglycemia, response_n),
      decimals = 0,
      use_seps = TRUE
    ) %>%
    gt::fmt_number(
      columns = c(normoglycemia_pct, hyperglycemia_pct),
      decimals = 1
    ) %>%
    gt::tab_spanner(
      label = "Binary glycemic distribution",
      columns = c(Normoglycemia, normoglycemia_pct, Hyperglycemia, hyperglycemia_pct)
    )
}
...
## ----survey-hyperglycemic-prior-a1c-awareness-assoc-prep, warning=FALSE, message=FALSE, include=FALSE----
survey_hyper_prior_a1c_assoc_data <- survey_hyper_prior_a1c_data %>%
  dplyr::mutate(
    lack_prior_a1c_awareness = q3_prior_a1c_awareness %in% c("No", "Unsure"),
    lack_prior_a1c_awareness_label = dplyr::if_else(
      lack_prior_a1c_awareness,
      "No or unsure",
      "Yes"
    ),
    raceeth = dplyr::case_when(
      is.na(raceeth) | trimws(raceeth) == "" ~ NA_character_,
...
survey_hyper_urban_lookup <- if (!inherits(urban_rural_strat_data, "list") || is.null(urban_rural_strat_data$error)) {
  urban_rural_strat_data %>%
    dplyr::filter(!is.na(zip5), urban_rural_group %in% c("Urban", "Rural")) %>%
    dplyr::distinct(zip5, urban_rural_group)
} else {
  NULL
}

survey_hyper_prior_a1c_assoc_data <- if (!is.null(survey_hyper_urban_lookup)) {
  survey_hyper_prior_a1c_assoc_data %>%
    dplyr::left_join(survey_hyper_urban_lookup, by = "zip5")
} else {
  survey_hyper_prior_a1c_assoc_data %>%
    dplyr::mutate(urban_rural_group = NA_character_)
}

survey_hyper_race_awareness_summary <- survey_hyper_prior_a1c_assoc_data %>%
  dplyr::filter(!is.na(raceeth)) %>%
  dplyr::group_by(raceeth) %>%
  dplyr::summarise(
    donors = dplyr::n(),
    lacking_awareness_n = sum(lack_prior_a1c_awareness, na.rm = TRUE),
    lacking_awareness_pct = 100 * mean(lack_prior_a1c_awareness, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  dplyr::arrange(dplyr::desc(lacking_awareness_pct))

survey_hyper_urban_awareness_summary <- survey_hyper_prior_a1c_assoc_data %>%
  dplyr::filter(urban_rural_group %in% c("Urban", "Rural")) %>%
  ...
  dplyr::arrange(urban_rural_group)

survey_hyper_race_urban_awareness_summary <- survey_hyper_prior_a1c_assoc_data %>%
  dplyr::filter(!is.na(raceeth), urban_rural_group %in% c("Urban", "Rural")) %>%
  dplyr::group_by(urban_rural_group, raceeth) %>%
  dplyr::summarise(
    donors = dplyr::n(),
    lacking_awareness_n = sum(lack_prior_a1c_awareness, na.rm = TRUE),
    lacking_awareness_pct = 100 * mean(lack_prior_a1c_awareness, na.rm = TRUE),
  ...
  dplyr::arrange(urban_rural_group, raceeth)

survey_hyper_race_awareness_chisq <- tryCatch(
  chisq.test(
    xtabs(
      ~ raceeth + lack_prior_a1c_awareness_label,
      data = survey_hyper_prior_a1c_assoc_data %>% dplyr::filter(!is.na(raceeth))
    )
  ),
  error = function(e) NULL
)

survey_hyper_urban_awareness_chisq <- tryCatch(
  chisq.test(
    xtabs(
      ~ urban_rural_group + lack_prior_a1c_awareness_label,
      data = survey_hyper_prior_a1c_assoc_data %>%
        dplyr::filter(urban_rural_group %in% c("Urban", "Rural"))
    )
  ),
  error = function(e) NULL
...
survey_hyper_race_urban_awareness_chisq <- tryCatch(
  chisq.test(
    xtabs(
      ~ subgroup + lack_prior_a1c_awareness_label,
      data = survey_hyper_prior_a1c_assoc_data %>%
        dplyr::filter(!is.na(raceeth), urban_rural_group %in% c("Urban", "Rural")) %>%
        dplyr::mutate(subgroup = interaction(raceeth, urban_rural_group, sep = " | ", drop = TRUE))
    )
  ),
  error = function(e) NULL
)

survey_hyper_awareness_assoc_plot_data <- dplyr::bind_rows(
  survey_hyper_race_awareness_summary %>%
    dplyr::transmute(
      panel = "Race/ethnicity",
      group = raceeth,
      donors,
      lacking_awareness_pct
    ),
  survey_hyper_urban_awareness_summary %>%
    dplyr::transmute(
      panel = "Urbanicity",
      group = as.character(urban_rural_group),
      donors,
      lacking_awareness_pct
    )
)
...
## ----survey-glycemic-overview, warning=FALSE, message=FALSE, echo=FALSE-------
survey_hyperglycemic_overview %>%
  dplyr::mutate(
    donors = scales::comma(donors),
    highest_hyperglycemia_pct = round(highest_hyperglycemia_pct, 1),
    chisq_p = scales::pvalue(chisq_p_value, accuracy = 0.001)
  ) %>%
  gt::gt() %>%
  gt::tab_header(
    title = "Survey question summary",
    subtitle = "For each survey item, the table shows the response category with the highest observed hyperglycemia prevalence."
  ) %>%
  gt::cols_label(
    question = "Survey question",
    donors = "Donors",
    highest_hyperglycemia_response = "Response with highest hyperglycemia burden",
    highest_hyperglycemia_pct = "Hyperglycemia %",
    chisq_p = "Chi-square p"
  ) %>%
  gt::fmt_number(columns = highest_hyperglycemia_pct, decimals = 1)


## ----survey-hyperglycemic-prior-a1c-awareness-pictogram, warning=FALSE, message=FALSE, echo=FALSE, results='asis'----
if (knitr::is_html_output()) {
  icon_counts <- c(
    No = round(survey_hyper_prior_a1c_summary$no_pct),
    Unsure = round(survey_hyper_prior_a1c_summary$unsure_pct),
    Yes = round(survey_hyper_prior_a1c_summary$yes_pct)
  )
  icon_counts["Yes"] <- icon_counts["Yes"] + (100 - sum(icon_counts))
  ...
  icon_labels <- c(
    No = paste0("Unaware: ", sprintf("%.1f", survey_hyper_prior_a1c_summary$no_pct), "%"),
    Unsure = paste0("Unsure: ", sprintf("%.1f", survey_hyper_prior_a1c_summary$unsure_pct), "%"),
    Yes = paste0("Aware: ", sprintf("%.1f", survey_hyper_prior_a1c_summary$yes_pct), "%")
  )

  icon_sequence <- c(
    rep("No", icon_counts["No"]),
    rep("Unsure", icon_counts["Unsure"]),
    rep("Yes", icon_counts["Yes"])
  )
  ...
  legend_items <- lapply(
    ...
    function(category) {
      htmltools::tags$div(
        style = "display:flex; align-items:center; gap:8px; margin-right:16px;",
        htmltools::tags$span(
          style = paste0("color:", icon_colors[[category]], "; font-size:20px; line-height:1;"),
          htmltools::HTML("&#128100;")
        ),
        htmltools::tags$span(icon_labels[[category]])
      )
    }
  )
  ...
  htmltools::tagList(
    htmltools::tags$div(
      ...
      htmltools::tags$div(
        ...
        "Pictogram of prior A1c awareness among hyperglycemic donors"
      ),
      htmltools::tags$div(
        style = "color:#4B5563; margin-bottom:10px;",
        "Each icon represents approximately 1% of hyperglycemic donors with a nonmissing q3 response."
      ),
      htmltools::tags$div(
...
## ----survey-hyperglycemic-prior-a1c-awareness-snapshot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=7.5, fig.height=4.8----
cat(
  paste0(
    "Among ", scales::comma(survey_hyper_prior_a1c_summary$hyper_n),
    " hyperglycemic donors with a nonmissing response to q3, ",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$no_pct),
    "% reported that they did not know their A1c value before the March 2025 screening, ",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$unsure_pct),
    "% were unsure, and ",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$yes_pct),
    "% reported prior awareness. Overall, ",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$unaware_or_unsure_pct),
    "% were not definitively aware of their A1c before screening. The `a1c` field in this survey dataset is the March 2025 screening result rather than a pre-screening value; in this hyperglycemic group, the screening A1c had median ",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$current_a1c_median),
    "% (IQR ",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$current_a1c_q1),
    "%-",
    sprintf("%.1f", survey_hyper_prior_a1c_summary$current_a1c_q3),
    "%)."
  )
)

ggplot(
  survey_hyper_prior_a1c_plot_data,
  aes(x = category, y = percentage, fill = category)
) +
  ...
  coord_flip() +
  labs(
    title = "Prior A1c awareness among hyperglycemic donors before their screening",
    subtitle = "Hyperglycemia includes both prediabetes and diabetes;<br>survey question asked whether donors already knew their own A1c",
    x = "Percentage of donors",
    y = NULL,
    caption = "Based on survey responses of donors screened between March 2025-Dec 2025"
  ) +
  customeTheme +
  theme(
...
## ----survey-hyperglycemic-prior-a1c-awareness-associations, warning=FALSE, message=FALSE, echo=FALSE, fig.width=9, fig.height=4.8----
race_p_text <- if (!is.null(survey_hyper_race_awareness_chisq)) {
  scales::pvalue(survey_hyper_race_awareness_chisq$p.value, accuracy = 0.001)
} else {
  "not available"
}

urban_p_text <- if (!is.null(survey_hyper_urban_awareness_chisq)) {
  scales::pvalue(survey_hyper_urban_awareness_chisq$p.value, accuracy = 0.001)
} else {
  "not available"
}
...
cat(
  paste0(
    "Among hyperglycemic donors with a nonmissing q3 response, the proportion lacking definite prior A1c awareness ",
    "(responding no or unsure) varied by race/ethnicity and by urban-rural residence. ",
    "The race/ethnicity association had chi-square p ",
    race_p_text,
    ", and the urban-rural association had chi-square p ",
    urban_p_text,
    "."
  )
)

awareness_plot_data <- dplyr::bind_rows(
  survey_hyper_race_awareness_summary %>%
    dplyr::transmute(
      panel = "Race/ethnicity",
      subgroup = as.character(raceeth),
      lacking_awareness_pct = lacking_awareness_pct
    ),
  survey_hyper_urban_awareness_summary %>%
    dplyr::transmute(
      panel = "Urbanicity",
      subgroup = as.character(urban_rural_group),
      lacking_awareness_pct = lacking_awareness_pct
    )
) %>%
  dplyr::mutate(
    panel = factor(panel, levels = c("Race/ethnicity", "Urbanicity")),
    subgroup = factor(
      subgroup,
      levels = unique(c(
        as.character(survey_hyper_race_awareness_summary$raceeth),
        as.character(survey_hyper_urban_awareness_summary$urban_rural_group)
      ))
    )
  )

awareness_fill_colors <- c(
  "Asian/AIAN/other" = "#C7362A",
  "Black" = "#E19346",
  "White" = "#F0D16A",
  "Hispanic Origin" = "#C98800",
  "Urban" = "#FDB04A",
  "Rural" = "#FDE79B"
)

awareness_axis_limit <- max(
  60,
  5 * ceiling((max(awareness_plot_data$lacking_awareness_pct, na.rm = TRUE) + 7) / 5)
)

ggplot(
  awareness_plot_data,
  aes(x = subgroup, y = lacking_awareness_pct, fill = subgroup, color = subgroup)
) +
  geom_col(width = 0.7, show.legend = FALSE) +
  facet_wrap(~ panel, scales = "free_y") +
  geom_text(
    aes(label = paste0(round(lacking_awareness_pct, 1), "%")),
    hjust = -0.1,
    size = 3.5,
    show.legend = FALSE
  ) +
  coord_flip() +
  scale_fill_manual(values = awareness_fill_colors, drop = FALSE) +
  scale_color_manual(values = awareness_fill_colors, drop = FALSE) +
  scale_y_continuous(
    labels = function(x) paste0(x, "%"),
    limits = c(0, awareness_axis_limit)
  ) +
  labs(
    title = "Lack of prior A1c awareness among hyperglycemic donors",
    subtitle = paste(
      "Lack of awareness defined as q3 response of No or Unsure;",
      "race/ethnicity p",
      race_p_text,
      "and urbanicity p",
      urban_p_text
    ),
    caption = "County type assigned from donor ZIP linked to county urban-rural classification",
    x = NULL,
    y = "Hyperglycemic donors lacking definite prior A1c awareness (%)"
  ) +
  customeTheme +
  theme(
    axis.text.x = element_blank(),
...
survey_hyper_race_awareness_summary %>%
  dplyr::mutate(donors = scales::comma(donors)) %>%
  gt::gt() %>%
  gt::tab_header(
    title = "By race/ethnicity",
    subtitle = paste("Chi-square p", race_p_text)
  ) %>%
  gt::cols_label(
    raceeth = "Race/ethnicity",
    donors = "Hyperglycemic donors",
    lacking_awareness_n = "No or unsure n",
    lacking_awareness_pct = "No or unsure %"
  ) %>%
  gt::fmt_number(columns = lacking_awareness_n, decimals = 0) %>%
  gt::fmt_number(columns = lacking_awareness_pct, decimals = 1)

survey_hyper_urban_awareness_summary %>%
  dplyr::mutate(donors = scales::comma(donors)) %>%
  gt::gt() %>%
  gt::tab_header(
    title = "By urban-rural residence",
    subtitle = paste("Chi-square p", urban_p_text)
  ) %>%
  gt::cols_label(
...
## ----survey-hyperglycemic-prior-a1c-awareness-race-urban-bivariate, warning=FALSE, message=FALSE, echo=FALSE, fig.width=8.5, fig.height=4.8----
race_urban_p_text <- if (!is.null(survey_hyper_race_urban_awareness_chisq)) {
  scales::pvalue(survey_hyper_race_urban_awareness_chisq$p.value, accuracy = 0.001)
} else {
  "not available"
}

if (nrow(survey_hyper_race_urban_awareness_summary) > 0) {
  race_urban_segment_data <- survey_hyper_race_urban_awareness_summary %>%
    dplyr::mutate(
      raceeth = factor(
        as.character(raceeth),
        levels = c("White", "Black", "Hispanic Origin", "Asian/AIAN/other")
      ),
      urban_rural_group = factor(
        as.character(urban_rural_group),
        levels = c("Urban", "Rural")
      )
    ) %>%
    dplyr::select(raceeth, urban_rural_group, lacking_awareness_pct) %>%
    tidyr::pivot_wider(
      names_from = urban_rural_group,
      values_from = lacking_awareness_pct
    ) %>%
    dplyr::filter(!is.na(Urban), !is.na(Rural))

  ggplot(
    survey_hyper_race_urban_awareness_summary,
    aes(
      x = lacking_awareness_pct,
      y = raceeth,
      color = urban_rural_group,
      size = donors
    )
  ) +
    geom_segment(
      data = race_urban_segment_data,
      aes(
        x = Urban,
        xend = Rural,
        y = raceeth,
    ...
    scale_color_manual(
      values = c("Urban" = "#F6A03A", "Rural" = "#B35806"),
      name = "County type"
    ) +
    scale_y_discrete(limits = c("White", "Black", "Hispanic Origin", "Asian/AIAN/other")) +
    scale_x_continuous(
      labels = function(x) paste0(x, "%"),
      limits = c(0, 60)
    ) +
    scale_size_continuous(range = c(3, 8), labels = scales::comma) +
    labs(
      title = "Lack of prior A1c awareness among hyperglycemic donors by race and urbanicity",
      subtitle = paste(
        "Points show subgroup percentages and size reflects hyperglycemic donor count;",
        "combined subgroup p", race_urban_p_text
      ),
      x = "Hyperglycemic donors unaware or unsure of prior A1c (%)",
      y = "Race/ethnicity",
      size = "Hyperglycemic donors"
    ) +
    customeTheme +
    theme(
      panel.grid.major.y = element_blank(),
  ...
  survey_hyper_race_urban_awareness_summary %>%
    ...
    gt::gt() %>%
    gt::tab_header(
      title = "Race by urban-rural residence",
      subtitle = paste("Combined subgroup chi-square p", race_urban_p_text)
    ) %>%
    gt::cols_label(
      urban_rural_group = "County type",
      raceeth = "Race/ethnicity",
  ...
  cat("Race-by-urbanicity subgroup data were not available in this session.")
}


## ----survey-q1-a1c-knowledge, warning=FALSE, message=FALSE, echo=FALSE--------
render_survey_hyperglycemic_gt("q1_a1c_knowledge")


## ----survey-q3-prior-a1c-awareness, warning=FALSE, message=FALSE, echo=FALSE----
render_survey_hyperglycemic_gt("q3_prior_a1c_awareness")
...
donor_cluster_results <- tryCatch({
  ...
  cluster_glycemic_summary <- cluster_glycemic %>%
    dplyr::filter(glycemic_category %in% c("Prediabetes", "Diabetes")) %>%
    dplyr::select(cluster, glycemic_category, percentage) %>%
    tidyr::pivot_wider(
      names_from = glycemic_category,
      values_from = percentage,
      names_glue = "{glycemic_category}_pct"
    )

  cluster_cardiorespiratory_summary <- donor_cluster_input %>%
    dplyr::group_by(cluster) %>%
    dplyr::summarise(
      heart_lung_condition_present_pct = mean(
        cardiorespiratory_history == "History of heart/lung condition",
        na.rm = TRUE
      ),
      .groups = "drop"
    )
  ...
  cluster_size_tbl <- donor_cluster_input %>%
    ...
    dplyr::mutate(percentage = donors / sum(donors)) %>%
    dplyr::left_join(cluster_glycemic_summary, by = "cluster")

  cluster_modal_profile <- cluster_modal_profile %>%
    dplyr::select(-`Heart/lung condition`) %>%
    dplyr::left_join(cluster_cardiorespiratory_summary, by = "cluster") %>%
    dplyr::mutate(
      `Heart/lung condition present` = scales::percent(
        heart_lung_condition_present_pct,
        accuracy = 0.1
      )
    ) %>%
    dplyr::select(-heart_lung_condition_present_pct)
...
## ----donor-kmeans-cluster-output, warning=FALSE, message=FALSE, echo=FALSE, results='asis', fig.width=11, fig.height=8----
if (!is.null(donor_cluster_results$error)) {
  cat(
    "Donor clustering analysis unavailable.\n\n",
    "Reason: ", donor_cluster_results$error, "\n",
    sep = ""
  )
} else {
  cat(
    ...
    knitr::kable(
      donor_cluster_results$cluster_size_tbl %>%
        dplyr::mutate(
          donors = scales::comma(donors),
          percentage = scales::percent(percentage, accuracy = 0.1),
          Prediabetes_pct = scales::percent(Prediabetes_pct, accuracy = 0.1),
          Diabetes_pct = scales::percent(Diabetes_pct, accuracy = 0.1)
        ) %>%
        dplyr::rename(
          Cluster = cluster,
          Donors = donors,
          `Donor share` = percentage,
          `Prediabetes %` = Prediabetes_pct,
  ...
  print(
    knitr::kable(
      ...
      caption = "Dominant donor profile within each k-means cluster, with percentage having a heart/lung condition."
    )
  )
}
...
## ----donor-diabetes-tree-prep, warning=FALSE, message=FALSE, include=FALSE----
donor_tree_results <- tryCatch({
  if (inherits(urban_rural_strat_data, "list") && !is.null(urban_rural_strat_data$error)) {
    stop(urban_rural_strat_data$error)
  }

  donor_tree_input <- urban_rural_strat_data %>%
    dplyr::mutate(
      donor_type = dplyr::case_when(
        ftd == "Y" ~ "First-time donor",
        ftd == "N" ~ "Repeat donor",
      ...
      coagulopathy_history = dplyr::case_when(
        coagulopathy == 1 ~ "History of coagulopathy",
        coagulopathy == 0 ~ "No coagulopathy",
        TRUE ~ NA_character_
      ),
      anticoagulant_use = dplyr::case_when(
        anticoagulants == 1 ~ "On anticoagulants",
        anticoagulants == 0 ~ "Not on anticoagulants",
        TRUE ~ NA_character_
      ),
      deferral_med_history = dplyr::case_when(
        deferral_meds == 1 ~ "On deferral medications",
        deferral_meds == 0 ~ "Not on deferral medications",
        TRUE ~ NA_character_
      ),
      splenectomy_history = dplyr::case_when(
        splenectomy_tcpenia == 1 ~ "History of splenectomy/thrombocytopenia",
        splenectomy_tcpenia == 0 ~ "No splenectomy/thrombocytopenia history",
        TRUE ~ NA_character_
      ),
      statefp = substr(GEOID, 1, 2),
      region = dplyr::case_when(
      ...
      diabetes_outcome = factor(
        dplyr::if_else(glycemic_category == "Diabetes", "Diabetes", "No diabetes"),
        levels = c("No diabetes", "Diabetes")
      ),
      diabetes_flag = as.numeric(glycemic_category == "Diabetes"),
      age_group = factor(age_group, levels = c("-29", "30-54", "55-")),
      gender = factor(gender, levels = c("F", "M")),
      raceeth = factor(raceeth, levels = c("White", "Black", "Hispanic Origin", "Asian/AIAN/other")),
      donor_type = factor(donor_type, levels = c("First-time donor", "Repeat donor")),
      ...
      cancer_history = factor(cancer_history, levels = c("No history of cancer", "History of cancer")),
      cardiorespiratory_history = factor(
        cardiorespiratory_history,
        levels = c("No heart/lung condition", "History of heart/lung condition")
      ),
      coagulopathy_history = factor(
        coagulopathy_history,
        levels = c("No coagulopathy", "History of coagulopathy")
      ),
      anticoagulant_use = factor(
        anticoagulant_use,
        levels = c("Not on anticoagulants", "On anticoagulants")
      ),
      deferral_med_history = factor(
        deferral_med_history,
        levels = c("Not on deferral medications", "On deferral medications")
      ),
      splenectomy_history = factor(
        splenectomy_history,
        levels = c("No splenectomy/thrombocytopenia history", "History of splenectomy/thrombocytopenia")
      ),
      urban_rural_group = factor(urban_rural_group, levels = c("Urban", "Rural"))
    ) %>%
    dplyr::filter(
      !is.na(diabetes_outcome),
      !is.na(age_group),
      !is.na(gender),
      !is.na(raceeth),
      !is.na(donor_type),
      ...
      !is.na(cardiorespiratory_history),
      !is.na(coagulopathy_history),
      !is.na(anticoagulant_use),
      !is.na(deferral_med_history),
      !is.na(splenectomy_history)
    )

  if (nrow(donor_tree_input) < 1000) {
    stop("Too few complete-case donors were available for decision tree analysis.")
  }

  tree_formula <- diabetes_flag ~ age_group + gender + raceeth + donor_type +
    region + urban_rural_group + combined_hypertension_status + cancer_history +
    cardiorespiratory_history + coagulopathy_history + anticoagulant_use +
    deferral_med_history + splenectomy_history

  set.seed(42)
  training_n <- min(100000, nrow(donor_tree_input))
  training_idx <- sample.int(nrow(donor_tree_input), size = training_n)
  tree_training_data <- donor_tree_input[training_idx, , drop = FALSE]
  ...
  cp_table <- as.data.frame(tree_fit$cptable)
  best_cp_row <- which.min(cp_table$xerror)
  best_nonroot_row <- cp_table %>%
    tibble::rownames_to_column("cp_row") %>%
    dplyr::mutate(cp_row = as.integer(cp_row)) %>%
    dplyr::filter(nsplit > 0) %>%
    dplyr::arrange(xerror, xstd, dplyr::desc(nsplit)) %>%
    dplyr::slice_head(n = 1) %>%
    dplyr::pull(cp_row)

  pruned_tree <- rpart::prune(tree_fit, cp = cp_table[best_cp_row, "CP"])

  if (nrow(pruned_tree$frame) <= 1 && length(best_nonroot_row) == 1) {
    pruned_tree <- rpart::prune(tree_fit, cp = cp_table[best_nonroot_row, "CP"])
  }

  donor_tree_input <- donor_tree_input %>%
    dplyr::mutate(
      node_id = rpart:::pred.rpart(
        pruned_tree,
        rpart:::rpart.matrix(
          stats::model.frame(
            stats::delete.response(pruned_tree$terms),
            donor_tree_input,
            na.action = stats::na.pass,
            xlev = attr(pruned_tree, "xlevels")
          )
        )
      )
    )

  tree_terminal_summary <- donor_tree_input %>%
    dplyr::group_by(node_id) %>%
    dplyr::summarise(
      donors = dplyr::n(),
      diabetes_pct = mean(diabetes_flag, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    dplyr::arrange(dplyr::desc(diabetes_pct), dplyr::desc(donors)) %>%
    dplyr::mutate(
      terminal_node = paste0("Node ", node_id),
      donor_share = donors / sum(donors)
    )

  tree_terminal_profiles <- donor_tree_input %>%
    dplyr::count(node_id, age_group, gender, raceeth, donor_type, region, urban_rural_group,
      combined_hypertension_status, cancer_history, cardiorespiratory_history,
      name = "donors"
    ) %>%
    dplyr::group_by(node_id) %>%
    dplyr::slice_max(order_by = donors, n = 1, with_ties = FALSE) %>%
    dplyr::ungroup() %>%
    dplyr::transmute(
      node_id,
      `Age group` = age_group,
      ...
      Region = region,
      `County type` = urban_rural_group,
      `Hypertension status` = combined_hypertension_status,
      `Cancer history` = cancer_history,
      `Heart/lung condition` = cardiorespiratory_history
    ) %>%
    dplyr::right_join(
      tree_terminal_summary %>% dplyr::select(node_id, terminal_node),
      by = "node_id"
    ) %>%
    dplyr::select(terminal_node, dplyr::everything(), -node_id)

  tree_variable_importance_source <- if (!is.null(pruned_tree$variable.importance)) {
    pruned_tree$variable.importance
  } else {
    tree_fit$variable.importance
  }

  tree_variable_importance <- if (is.null(tree_variable_importance_source)) {
    tibble::tibble(
      variable = character(),
      importance = numeric(),
    ...
    tibble::tibble(
      variable = names(tree_variable_importance_source),
      importance = as.numeric(tree_variable_importance_source)
    ) %>%
      dplyr::arrange(dplyr::desc(importance)) %>%
      dplyr::mutate(relative_importance = importance / max(importance))
  }
  ...
  list(
    ...
    tree_has_splits = nrow(pruned_tree$frame) > 1,
    tree_terminal_summary = tree_terminal_summary,
    tree_terminal_profiles = tree_terminal_profiles,
...
## ----donor-diabetes-tree-output, warning=FALSE, message=FALSE, echo=FALSE, results='asis', fig.width=12, fig.height=8----
if (!is.null(donor_tree_results$error)) {
  cat(
    "Decision tree analysis unavailable.\n\n",
    "Reason: ", donor_tree_results$error, "\n",
    sep = ""
  )
} else {
  cat(
    paste0(
      "A decision tree was fit to identify donor segments with higher observed diabetes prevalence using demographic, geographic, urbanicity, and medical-history determinants. ",
      "The terminal-node summaries below are based on ",
      scales::comma(nrow(donor_tree_results$donor_tree_input)),
      " complete-case donors assigned to the pruned tree."
    ),
    "\n\n"
  )
  ...
  print(
    knitr::kable(
      donor_tree_results$tree_terminal_summary %>%
        dplyr::mutate(
          donors = scales::comma(donors),
          donor_share = scales::percent(donor_share, accuracy = 0.1),
          diabetes_pct = scales::percent(diabetes_pct, accuracy = 0.1)
        ) %>%
        dplyr::select(
          `Terminal node` = terminal_node,
          Donors = donors,
          `Donor share` = donor_share,
          `Diabetes %` = diabetes_pct
        ),
      caption = "Terminal-node diabetes prevalence from the donor decision tree."
    )
  )
  ...
  print(
    knitr::kable(
      donor_tree_results$tree_terminal_profiles,
      caption = "Most common donor profile within each terminal node of the decision tree."
    )
  )

  cat("\n\n")

  if (nrow(donor_tree_results$tree_variable_importance) > 0) {
    print(
      knitr::kable(
        donor_tree_results$tree_variable_importance %>%
          dplyr::mutate(
            importance = round(importance, 2),
            relative_importance = scales::percent(relative_importance, accuracy = 0.1)
          ) %>%
          dplyr::rename(
            Variable = variable,
        ...
        caption = "Variable importance from the donor decision tree."
      )
    )
  } else {
    cat(
      "Variable importance from the donor decision tree was not available because the fitted tree did not retain any splitting variables.",
      "\n"
    )
  }
}
...
## ----donor-diabetes-tree-plot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=13, fig.height=9----
if (!is.null(donor_tree_results$error)) {
  plot.new()
  text(0.5, 0.55, "Decision tree figure unavailable.", cex = 1.1)
  text(0.5, 0.45, paste("Reason:", donor_tree_results$error), cex = 0.9)
} else {
  if (isTRUE(donor_tree_results$tree_has_splits)) {
    tree_plot <- donor_tree_results$pruned_tree
    ...
    if (requireNamespace("rpart.plot", quietly = TRUE)) {
      rpart.plot::rpart.plot(
        ...
        shadow.col = 0,
        nn = FALSE,
        main = "Decision tree for donor diabetes prevalence"
      )
    } else {
      plot(tree_plot, uniform = TRUE, margin = 0.08, main = "Decision tree for donor diabetes prevalence")
      text(tree_plot, use.n = TRUE, all = TRUE, cex = 0.72)
    }
  } else {
    plot.new()
    text(0.5, 0.56, "The pruned decision tree did not retain any splits in this session.", cex = 1.0)
    text(0.5, 0.47, "Variable-importance results are still shown below when available.", cex = 0.9)
  }
  ...
  if (nrow(donor_tree_results$tree_variable_importance) > 0) {
    importance_plot <- ggplot(
      ...
      aes(x = stats::reorder(variable, importance), y = importance)
    ) +
      geom_col(fill = "#B35806", width = 0.72) +
      coord_flip() +
      labs(
        title = "Decision-tree variable importance",
        subtitle = "Top predictors contributing to donor diabetes segmentation",
        x = NULL,
        y = "Importance"
      ) +
      theme_minimal(base_size = 11) +
...
hotspot_map_data <- tryCatch({
  ...
  svi_county <- readr::read_csv("SVI_2022_US_county.csv", show_col_types = FALSE) %>%
    dplyr::transmute(
      ...
      svi_theme1 = as.numeric(SPL_THEME1)
    )
    ...
    dplyr::mutate(
      ...
      popup_html = dplyr::case_when(
        ...
        TRUE ~ paste0(
          ...
          "<br/>Socioeconomic Status Score: ", ifelse(is.na(svi_theme1), "Missing", scales::number(svi_theme1, accuracy = 0.01)),
...
if (!is.null(hotspot_map_data$error)) {
  ...
  svi_theme1_values <- swipe_county_map$svi_theme1[!is.na(swipe_county_map$svi_theme1)]
  svi_theme1_max <- if (length(svi_theme1_values) > 0) max(svi_theme1_values, na.rm = TRUE) else 0
  svi_theme1_bins <- if (is.finite(svi_theme1_max) && svi_theme1_max > 0) {
    unique(seq(0, svi_theme1_max, length.out = 5))
  } else {
    c(0, 1)
  }
  ...
  svi_pal <- leaflet::colorBin(
    ...
    domain = c(0, svi_theme1_max),
    bins = svi_theme1_bins,
    na.color = "#efe9df"
  )
  ...
  swipe_county_map <- swipe_county_map %>%
    dplyr::mutate(
      ...
      svi_swipe_fill = svi_pal(svi_theme1),
  ...
  swipe_leaflet_widget <- leaflet::leaflet(
    ...
    leaflet::addControl(
      html = as.character(
        htmltools::tags$div(
          ...
          htmltools::tags$div("Right: county SVI socioeconomic status score")
        )
      ),
      position = "topright"
    ) %>%
    ...
    leaflet::addLegend(
      position = "bottomright",
      pal = svi_pal,
      values = swipe_county_map$svi_theme1,
      title = "Socioeconomic Status Score",
      opacity = 1,
...
if (is.null(hotspot_map_data$error)) {
  ...
  county_assoc_theme1 <- county_assoc %>%
    dplyr::filter(!is.na(svi_theme1))

  socioecon_assoc_test <- cor.test(
    county_assoc_theme1$diabetes_prev,
    county_assoc_theme1$svi_theme1,
    method = "spearman",
    exact = FALSE
  )

  survey_hyper_awareness_county <- survey_a1c_demo %>%
    dplyr::filter(
      glycemic_category %in% c("Prediabetes", "Diabetes"),
      !is.na(q3_prior_a1c_awareness)
    ) %>%
    ...
    dplyr::group_by(GEOID) %>%
    dplyr::summarise(
      hyperglycemic_survey_donors = dplyr::n(),
      unaware_hyperglycemic_donors = sum(lack_prior_a1c_awareness, na.rm = TRUE),
      unaware_hyperglycemic_pct = 100 * mean(lack_prior_a1c_awareness, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    dplyr::left_join(
      hotspot_map_data$county_map %>%
        sf::st_drop_geometry() %>%
        dplyr::distinct(GEOID, county_label, svi_theme1),
      by = "GEOID"
    ) %>%
    dplyr::filter(!is.na(svi_theme1))

  survey_hyper_awareness_county_theme1 <- survey_hyper_awareness_county %>%
    dplyr::filter(hyperglycemic_survey_donors >= 10)

  survey_hyper_awareness_theme1_test <- if (nrow(survey_hyper_awareness_county_theme1) >= 3) {
    cor.test(
      survey_hyper_awareness_county_theme1$unaware_hyperglycemic_pct,
      survey_hyper_awareness_county_theme1$svi_theme1,
      method = "spearman",
      exact = FALSE
    )
  } else {
...
## ----county-socioeconomic-scatterplot, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8.5, fig.height=6----
if (is.null(hotspot_map_data$error)) {
  county_assoc_theme1_plot <- ggplot(
    county_assoc_theme1,
    aes(x = svi_theme1, y = diabetes_prev)
  ) +
    geom_smooth(
      method = "lm",
      se = TRUE,
    ...
    scale_y_continuous(labels = function(x) paste0(x, "%")) +
    labs(
      title = "County donor diabetes prevalence vs. socioeconomic status score",
      subtitle = paste0(
        "Spearman rho = ",
        round(unname(socioecon_assoc_test$estimate), 3),
        ", p = ",
        scales::pvalue(socioecon_assoc_test$p.value, accuracy = 0.001)
      ),
      x = "Socioeconomic Status Score",
      y = "Diabetic donors (%)",
      size = "Donors per county",
      color = "Obese donors (%)"
    ) +
...
## ----county-hyperglycemic-unawareness-socioeconomic-scatterplot, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8.5, fig.height=6----
if (is.null(hotspot_map_data$error)) {
  if (!is.null(survey_hyper_awareness_theme1_test) && nrow(survey_hyper_awareness_county_theme1) > 0) {
    cat(
      paste0(
        "Across ", scales::comma(nrow(survey_hyper_awareness_county_theme1)),
        " counties with at least 10 hyperglycemic survey donors and nonmissing socioeconomic status score, the county-level percentage of hyperglycemic donors who were unaware or unsure of their A1c before screening was evaluated against county SPL_THEME1."
      )
    )

    ggplot(
      survey_hyper_awareness_county_theme1,
      aes(x = svi_theme1, y = unaware_hyperglycemic_pct)
    ) +
      geom_smooth(
        method = "lm",
        se = TRUE,
      ...
      geom_point(
        aes(size = hyperglycemic_survey_donors),
        color = "#C05A2B",
        alpha = 0.6
      ) +
      ...
      scale_y_continuous(labels = function(x) paste0(x, "%")) +
      labs(
        title = "County hyperglycemic donor unawareness vs. socioeconomic status score",
        subtitle = paste0(
          "Spearman rho = ",
          round(unname(survey_hyper_awareness_theme1_test$estimate), 3),
          ", p = ",
          scales::pvalue(survey_hyper_awareness_theme1_test$p.value, accuracy = 0.001)
        ),
        x = "Socioeconomic Status Score (SPL_THEME1)",
        y = "Hyperglycemic donors unaware or unsure of prior A1c (%)",
        size = "Hyperglycemic survey donors per county",
        caption = "Counties are restricted to those with at least 10 hyperglycemic survey donors and nonmissing county SPL_THEME1."
      ) +
      theme_minimal(base_size = 12) +
      theme(
        panel.grid.minor = element_blank(),
    ...
    cat("County-level hyperglycemic unawareness correlation could not be estimated in this session.")
  }
}
